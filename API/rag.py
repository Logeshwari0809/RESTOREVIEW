...
def get_res_reviews(name):
    ...
    res = es.search(index="review_index", body=query,source=['restaurant_name','review'])
    documents = []
    for doc in res['hits']['hits']:
        documents.append(str(doc['_source']))
    return documents
...
def get_summary(name):
    ...
    prompt = f"You are a resturant suggestor. you provide summary of the given restaurant:'{name}' based on given review_index.\n\review_index:\n{''.join(documents)}. output format{output_format}"
    response = model.generate_content(prompt)
    return response.text
...
def restaurant_qna(name, query):
    ...
    prompt = f"You are a resturant suggestor.Answer the user's query:'{query} on a restaurant':'{name}' based on given review_index.\n\review_index:\n{''.join(documents)}. output format: {output_format}"
    response = model.generate_content(prompt)
    return response.text
